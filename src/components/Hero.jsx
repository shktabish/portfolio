  ...
  return (
    <div id="hero" className="h-screen w-full flex flex-col justify-center items-center">
        <div onMouseEnter={() => setIsActive(true)} onMouseLeave={() => setIsActive(false)} 
            className="demo text-7xl max-sm:text-5xl font-Grotesk relative z-50"
        >
            Hello I am,
        </div>
        <div onMouseEnter={() => setIsActive(true)} onMouseLeave={() => setIsActive(false)} 
            className="demo text-9xl max-sm:text-7xl font-Grotesk relative z-50"
        >
            Tabish Shaikh
        </div>
        <div onMouseEnter={() => setIsActive(true)} onMouseLeave={() => setIsActive(false)} 
            className="text-7xl max-sm:text-5xl text-center font-Chakra font-thin my-4 relative z-50"
        >
            {text}
        </div>
    </div>
